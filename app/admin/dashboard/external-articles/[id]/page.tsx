...
export default function EditExternalArticlePage({
  params,
}: {
  ...
  const [formData, setFormData] = useState<ExternalArticle | null>(null);
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.article.image_url) {
          setImagePreview(data.article.image_url);
        }
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/external-articles/${articleId}`, {
        method: 'PUT',
        headers: {
      ...
      console.error('Error updating external article:', error);
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-8 text-center">
          <p className="text-red-400">{error || 'Article non trouvé'}</p>
          <Link
            href="/admin/dashboard/external-articles"
            className="inline-block mt-4 text-white hover:text-[var(--gold)] transition-colors"
          >
            Retour aux articles
          </Link>
        </div>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-white/60">
          Modifiez les informations de l&apos;article externe
        </p>
      </div>
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-[var(--orange-light)]/20 rounded-xl p-8 space-y-6">
          ...
          <div>
            <label className="flex items-center cursor-pointer">
              ...
              />
              <span className="ml-3 text-white">Publier</span>
            </label>
            <p className="text-xs text-white/50 mt-2 ml-8">
              Décochez pour masquer l&apos;article
            </p>
          </div>
